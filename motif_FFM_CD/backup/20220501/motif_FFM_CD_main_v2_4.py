...
#        (nmm_pop, nmm_fit) = func.MNMM_V2(new_pop, n, c, NP, adj, motif_adj, threshold_value) 
    ...
    if len(set(bestX_membership)) == 3:
        logs_path = r"./logs/motif_SOSFCD/zhang/zhang_Qg_log.txt"
        ...
        run += 1
        # 保存目前n次的独立实验中的最优值
        bestQ_path = r"./paras/motif_SOSFCD/zhang/bestQg.txt"
        flag = 0
        ...
        if flag == 1:
            ...
            with open(bestQ_path, mode='w',encoding='UTF-8') as f:
                f.write(str(bestQ))
            ## 更新最优解    
            pop_best_history_path = r"result/motif_SOSFCD/zhang/pop_best_Qg_history.txt"
            fit_best_history_path = r"result/motif_SOSFCD/zhang/fit_best_Qg_history.txt"
            nmi_path = r"result/motif_SOSFCD/zhang/NMI_Qg_history.txt"
