...
#        (nmm_pop, nmm_fit) = func.MNMM_V2(new_pop, n, c, NP, adj, motif_adj, threshold_value) 
    ...
    bestX = pop_best_history[:,:,99]
    bestX_membership = np.argmax(bestx, axis=0)
    # 保存每次独立实验的最优值到日志文件
    if len(set(bestX_membership)) == 4:
        logs_path = r"./logs/motif_SOSFCD/karate/karate_Qg_log.txt"
        with open(logs_path, mode='a+',encoding='UTF-8') as log_f:
            ...
            run += 1
    # 保存目前n次的独立实验中的最优值
    bestQ_path = r"./paras/motif_SOSFCD/karate/bestQg.txt"
    flag = 0
    ...
    if flag == 1:
        ...
        with open(bestQ_path, mode='w',encoding='UTF-8') as f:
            f.write(str(bestQ))
        ## 更新最优解    
        pop_best_history_path = r"result/motif_SOSFCD/karate/pop_best_Qg_history.txt"
        fit_best_history_path = r"result/motif_SOSFCD/karate/fit_best_Qg_history.txt"
        nmi_path = r"result/motif_SOSFCD/karate/NMI_Qg_history.txt"
