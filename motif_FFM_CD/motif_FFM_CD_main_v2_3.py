...
Independent_Runs = 49 # 本次实验独立运行次数
...
#            break
        if (gen+1) % 20 ==0:
            print("#####motif_SOSFCD_Qg_nmm#####")
            print("gen=",gen+1)
            print(membership_c)
            print("NMI=",nmi)
            print("bestQg_nmm=",best_Q)
...
#    if len(set(bestX_membership)) == 3:
    logs_path = r"./logs/motif_SOSFCD/zhang/zhang_Qg_nmm_log.txt"
    ...
    run += 1
    # 保存目前n次的独立实验中的最优值
    bestQ_path = r"./paras/motif_SOSFCD/zhang/bestQg_nmm.txt"
    flag = 0
    ...
    if flag == 1:
        ...
        with open(bestQ_path, mode='w',encoding='UTF-8') as f:
            f.write(str(bestQ))
        ## 更新最优解    
        pop_best_history_path = r"result/motif_SOSFCD/zhang/pop_best_Qg_nmm_history.txt"
        fit_best_history_path = r"result/motif_SOSFCD/zhang/fit_best_Qg_nmm_history.txt"
        nmi_path = r"result/motif_SOSFCD/zhang/NMI_Qg_nmm_history.txt"
