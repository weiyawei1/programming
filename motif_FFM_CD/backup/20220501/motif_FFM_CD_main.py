...
# 选择网络
net_list = [beican_9_network,karate_network,dolphins_network,football_network]
for index_net, net in enumerate(net_list):
    network = net
    ...
    fit_values = func.fit_Qgs(pop,n,c,NP,motif_adj)   #适应度函数值计算 
    
    #有偏操作
    bias_pop = func.bias_init_pop(pop, c, n, NP, adj) # 对初始化后的种群进行有偏操作
    bias_fit_values = func.fit_Qgs(bias_pop,n,c,NP,motif_adj)   #适应度函数值计算 
    #选择优秀个体并保留到种群
    ...
    for gen in range(Gen):
        ...
        # mnmm_fit = []
        # for i in range(NP):
        #     mnmm_fit.append(func.fit_Qg(nmm_pop[:,:,i],n,c,motif_adj))
        ...
        for index in range(NP):
            if nmm_fit[index] > new_fit[index]:
                ...
                better_number+=1
        print("nmm_better_number={}".format(better_number))
        print("nmm_best_Qg={}".format(max(new_fit)))
    ...
    # 保存历史信息到文件
    # =============================================================================
    pop_best_history_path = r"./result/"+ str(index_net) +"_pop_best_history.txt"
    fit_best_history_path = r"./result/"+ str(index_net) +"_fit_best_history.txt"
    nmi_path = r"./result/NMI.txt"
    try:
