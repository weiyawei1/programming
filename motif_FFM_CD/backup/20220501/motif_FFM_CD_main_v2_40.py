...
#        (new_pop, new_fit) = alg_func.SOSFCD(pop, fit_values, n, c, NP, adj)      
        ...
        new_fit = copy.deepcopy(fit_values)
        # NMM 操作
...
#        nmilist.append(nmi)
#        size = len(set(membership_c))
#        if size <3:
#            print(best_Q,size)
#            break
        if (gen+1) % 1 ==0:
            print("#####motif_SOSFCD_Qc_FCD_nmm#####")
            print("gen=",gen+1)
#            print("c_count=",len(set(membership_c)))
            print("membership=",membership_c)            
#            print("NMI=",nmi)
            print("bestQc_FCD_nmm=",best_Q)
            
    end = time.process_time()
    
#    print("sp end_time=",end - start)
    ...
    if len(set(bestX_membership)) == 3:
        logs_path = r"./logs/motif_SOSFCD/karate/karate_Qc_FCD_nmm_log.txt"
        ...
        run += 1
        # 保存目前n次的独立实验中的最优值
        bestQ_path = r"./paras/motif_SOSFCD/karate/bestQc_FCD_nmm.txt"
        flag = 0
        ...
        if flag == 1:
            ...
            with open(bestQ_path, mode='w',encoding='UTF-8') as f:
                f.write(str(bestQ))
            ## 更新最优解    
            pop_best_history_path = r"result/motif_SOSFCD/karate/pop_best_Qc_FCD_nmm_history.txt"
            fit_best_history_path = r"result/motif_SOSFCD/karate/fit_best_Qc_FCD_nmm_history.txt"
            nmi_path = r"result/motif_SOSFCD/karate/NMI_Qc_FCD_nmm_history.txt"
