...
fit_values = func.fit_Qs(pop,motif_adj,n,c,NP)   #适应度函数值计算 
# end = clock()
# print("spend_time=",end - start)

#有偏操作
bias_pop = func.bias_init_pop(pop, c, n, NP, adj) # 对初始化后的种群进行有偏操作
bias_fit_values = func.fit_Qs(bias_pop,motif_adj,n,c,NP)   #适应度函数值计算 
#选择优秀个体并保留到种群
...
bestQ_path = r"./paras/motif_SOSFCD/karate/nmm_bestQw.txt"
flag = 0
...
if flag == 1:
    ...
    with open(bestQ_path, mode='w',encoding='UTF-8') as f:
        f.write(str(bestQ))
    ## 更新最优解    
    pop_best_history_path = r"result/motif_SOSFCD/karate/nmm_Qw_pop_best_history.txt"
    fit_best_history_path = r"result/motif_SOSFCD/karate/nmm_Qw_fit_best_history.txt"
    nmi_path = r"result/motif_SOSFCD/karate/NMI_nmm_Qw_history.txt"
    try:
