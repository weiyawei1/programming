...
# =============================================================================
#     fit_Qgs: 计算种群中每个个体的模糊重叠社区划分的模块度函数Qg值
#     c: 社区划分的数目
#     n: 网络节点数目
#     NP： 种群个体数目
#     adj: 网络邻接矩阵
# =============================================================================
def fit_Qgs(pop,adj,n,c,NP):
    Qgs = []
    W = np.sum(adj) # 权值之和
...
#         for i in range(n):
#             for j in range(n):
#                 Qg = Qg + (adj[i,j] - (m[0,i]*m[0,j])/W)*X[k,i]*X[k,j]
#     Qg/=W
#     return Qg
...
def NMM(pop, n, c, NP, adj, motif_adj, threshold_value):
    ...
    nmm_fit = fit_Qgs(nmm_pop,motif_adj,n,c,NP)   #适应度函数值计算 
...
def find_unreasonableNodes(unreasonableNodes,pick,Xi,adj,c,n,threshold_value):
    for i in pick:
        ...
        # i_c != j_c
        cd_i = len(np.where(j_nodes_c != i_node_c)) / len(j_nodes)
...
# =============================================================================
#     MNMM: 基于邻居节点的社区修正（基于边邻居节点和模体邻居节点）
#     nmm_pop: 种群
#     c: 社区划分的数目
#     n: 网络节点数目
#     NP： 种群中的个体数
...
#     return: nmm_pop, nmm_fit nmm种群, 适应度值
# =============================================================================
def MNMM(pop, n, c, NP, adj, motif_adj, threshold_value):
    nmm_pop = copy.deepcopy(pop)
    for i in range(NP):
        seeds = [i for i in range(n)]
        rd.shuffle(seeds)
        ...
        # 寻找不合理划分的节点和其对应的邻居节点
        unreasonableNodes = []
        find_unreasonableNodes_motifadd(unreasonableNodes,pick,nmm_pop[:,:,i],adj,motif_adj,c,n,threshold_value)
        # 获得该节点应划分的社区号
        node_cnos = []
...
# =============================================================================
#     find_unreasonableNodes_motifadd: 寻找基于边的不合理划分的节点
#     pick: 一定数量的随机节点
#     Xi: 第i个个体的隶属度矩阵
#     adj: 加权网络的临界矩阵
#     c: 社区划分的数目
...
# =============================================================================
def find_unreasonableNodes_motifadd(unreasonableNodes,pick,Xi,adj,motif_adj,c,n,threshold_value):
    for i in pick:
        # 获得节点 i 所在的社区
        i_node_c = np.argmax(Xi[:,i])
...
def unreasonableNodes_revise(node_cno_list,nmm_pop,N):
    for i_c in node_cno_list:
        ...
        c = i_c[1]
        nmm_pop[c,i,N] += 0.5
        nmm_pop[:,i,N] /= np.sum(nmm_pop[:,i,N])
