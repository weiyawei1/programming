...
# # 设置全局变量
    ...
    # 获取真实社区划分列表
    real_mem = []
    with open(path + "/real/" + 'football_groundtruth.txt', mode='r',encoding='UTF-8') as f:
...
#            break
        if (gen+1) % 10 ==0:
            if nmm_flag == 0 and better_number == 0:
                nmm_flag = 1
                nmm_v2_count+=1
            elif nmm_flag == 1 and better_number == 0:
                nmm_flag = 0
                nmm_count+=1
            
            print("#####motif_SOSFCD_Q_nmm#####")
            print("gen=",gen+1)
            print("c_count=",len(set(membership_c)))
            print("NMI=",nmi)
            print("bestQ_nmm=",best_Q)
            print("nmm_count={}".format(nmm_count))
            print("nmm_v2_count={}".format(nmm_v2_count))
...
#    if len(set(bestX_membership)) == 3:
    logs_path = r"./logs/motif_SOSFCD/football/football_Q_nmm_log.txt"
    ...
    run += 1
    # 保存目前n次的独立实验中的最优值
    bestQ_path = r"./paras/motif_SOSFCD/football/bestQ_nmm.txt"
    flag = 0
    ...
    if flag == 1:
        ...
        with open(bestQ_path, mode='w',encoding='UTF-8') as f:
            f.write(str(bestQ))
        ## 更新最优解    
        pop_best_history_path = r"result/motif_SOSFCD/football/pop_best_Q_nmm_history.txt"
        fit_best_history_path = r"result/motif_SOSFCD/football/fit_best_Q_nmm_history.txt"
        nmi_path = r"result/motif_SOSFCD/football/NMI_Q_nmm_history.txt"
        
        try:
            if not os.path.exists(r"result/motif_SOSFCD/football"):
                os.makedirs(r"result/motif_SOSFCD/football") 
