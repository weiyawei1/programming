...
def SOSFCD(pop, fit_values, n, c, NP, adj):
    # Mutualism【互利共生】
    W = np.sum(adj) # 权值之和
    m = np.sum(adj, axis=0)[0:] # adj 各列之和
    ...
    for i in range(NP):
        ...
        # 适应度函数值计算
        Xinew_fit = func.fit_Qg(Xinew,adj,n,c,W,m)
        Xjnew_fit = func.fit_Qg(Xjnew,adj,n,c,W,m)
        # 选择优秀个体并保留到种群
    ...
    for i in range(NP):
        ...
        # 适应度函数值计算
        para_vector_fit = func.fit_Qg(para_vector,adj,n,c,W,m)
        # 选择优秀个体并保留到种群
